 ...
 */
route.post("/auth/signup", zValidator("json", authBodySchema), async (c) => {
    ...
    if (signInError || !signInData.session) {
        return c.json({ error: { message: signInError?.message ?? "Sign in after signup failed" } }, 500);
    }
 ...
 */
route.post("/auth/login", zValidator("json", authBodySchema), async (c) => {
    const { email, password } = c.req.valid("json");
